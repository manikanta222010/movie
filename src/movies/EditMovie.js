...
function UpdateMovie({ movie }) {
  ...
  const editMovie = () => {
    ...
    fetch(`https://61932a72d3ae6d0017da83c8.mockapi.io/movies/${movie.id}`,
      {
        method: "PUT",
  ...
  const formik = useFormik({  // const {handleSubmit, values, handleChange, handleBlur, errors, touched } we can use destructuring like this
    initialValues: { name: name, poster: poster, rating: rating, summary: summary, trailer: trailer },
  ...
  return (
    <form className="add-movie-form" onSubmit={formik.handleSubmit}>
      ...
      />
      <Button onClick={formik.errors === {} ? "" : formik.handleSubmit} variant="outlined">Update</Button>
    </form>
  );
}
